...
bool RobotServer::open(int port)
{
    // Check port number
...
bool RobotServer::isOpen()
{
    return socketServer >= 0;
}

int RobotServer::receive(char* data)
{
...
int RobotServer::send(char* data)
{
...
int RobotServer::sendAll(char* data)
{
    int total = 0;
    for (int i=0; i<numClients; i++)
        if(clients[i] != -1)
